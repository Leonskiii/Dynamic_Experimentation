...
function noun_of_length(len, used) {
    let result = "";
...
export class Return extends Term {
    ...
    print_into(array, _depth, _length) { array.push(" " + this.return_value + "\n"); }
    print_into_with_extra_lines(array, _depth, _size) { array.push(" " + this.return_value + "\n"); }
}
export class Nested_Ifs extends Term {
    ...
    set_condition_at_level(target_level, condition) {
        if (target_level == 1) {
        ...
        if (this.then_branch instanceof Return)
            throw "Invalid level";
        this.then_branch.set_condition_at_level(target_level - 1, condition);
    }
}
...
export function generate_if_statement_noise(nesting_depth, level) {
    const used = new Set();
    let returns = [];
...
export function generate_if_statement_no_noise(nesting_depth, level) {
    const used = new Set();
    let returns = [];
    for (let i = 1; i <= nesting_depth; i++)
        returns.push(i);
